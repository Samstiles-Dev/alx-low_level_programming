 ...
 * @pointer: pointer to memory allocated with malloc(fmr_size).
 * @fmr_size: size in bytes, of the allocated space for pointer
...
void *_realloc(void *pointer, unsigned int fmr_size, unsigned int latest_size)
{
	...
	if (latest_size > fmr_size)
	{
		...
		if (p == NULL)
			return (NULL);
		for (i = 0; i < fmr_size && i < latest_size; i++)
